...
- **ScriptSig vs Coinbase Tag** — The `ScriptSig` is the coinbase transaction's unlocking script; since there is no real UTXO to spend, it can carry arbitrary data (BIP-34 mandates a block-height prefix). The **Coinbase Tag** is just a small substring within that arbitrary data. It is specifically written by the pool to publicly claim the block, or to provide space for an extra nonce. This parser specifically targets this tag to reveal the pool's identity.
